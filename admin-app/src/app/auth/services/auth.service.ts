...
export class AuthService {
  ...
  public signUp(formData: SignUp): Observable<SignUpResponse> {
    return this.http
      .post<SignUpResponse>(`${baseUrl}/auth/sign-up`, formData)
      .pipe(
        tap((data) => {
  ...
  public signIn(formData: SignIn): Observable<SignInResponse> {
    return this.http
      .post<SignInResponse>(`${baseUrl}/auth/sign-in`, formData)
      .pipe(
        tap((data) => {
          this.saveInLocalStorage({
            token: data.token,
  ...
  public googleSignIn(token: string): Observable<SignInResponse> {
    //console.log({token})
    return this.http
      .post<SignInResponse>(`${baseUrl}/auth/google-sign-in`, { token })
      .pipe(
        tap((res) =>
          this.saveInLocalStorage({ token: res.token, email: res.user.email })
        )
      );
  }
  ...
  public verifyAndRefreshToken(): Observable<boolean> {
    ...
    const tokenInfo = localStorage.getItem('token')
      ...
      : ({} as TokenInfo);
    return this.http
      .get<SignInResponse>(`${baseUrl}/auth/verify-refresh-token`, {
        headers: {
