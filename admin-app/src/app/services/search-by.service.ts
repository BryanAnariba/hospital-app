...
export class SearchByService {
  ...
  private getDataToUsers (data: any, type: searchByType): User[] {
    switch (type) {
      case searchByType.USERS:
        return data.map((user: any) => new User(user.name, user.email, user.img, user.role, user.google, user.isActive, user.createdAt, user.updatedAt, user._id));
  ...
  public searchBy(type: searchByType, term: string) {
    ...
    return this.httpClient.get(url, this.authService.headers)
      .pipe(
        map(
          (data: any) => this.getDataToUsers(data, searchByType.USERS)
        )
      );
  }
}
